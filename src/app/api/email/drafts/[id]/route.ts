...
/** Soft-delete : status=cancelled — la carte disparaît définitivement. */
export const DELETE = withAuth(
  ...
  async (_request, auth, ctx: { params: Promise<{ id: string }> }) => {
    ...
    try {
      const cancelled = await cancelEmailDraft(id, userId);
      return Response.json({ ok: true, draft: await toEmailDraftPreview(cancelled) });
    } catch (error) {
      if (error instanceof EmailDraftError) {
        const status =
          error.code === "NOT_FOUND"

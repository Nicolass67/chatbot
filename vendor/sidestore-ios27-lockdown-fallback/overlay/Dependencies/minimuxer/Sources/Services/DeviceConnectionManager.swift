...
actor DeviceConnectionManager {
    ...
    private(set) var lastProbe: DeviceTransportProbeResult?
    ...
    nonisolated private func probeDevice(_ ip: String?) -> DeviceTransportProbeResult {
        let rpPort = gateway.getPort(for: .rppairing)
        let ldPort = gateway.getPort(for: .lockdown)
        guard let ip, !ip.isEmpty else {
            debugLog("[minimuxer] [iface] tcpProbe skipped — IP is nil or empty")
            return DeviceTransportProbeResult(
                remotePairingReachable: false,
                lockdownReachable: false,
                remotePairingPort: rpPort,
                lockdownPort: ldPort,
                selected: .unavailable
            )
        }

        debugLog("[transport] probing RPPairing")
        let rpReachable = NetworkUtils.testTCP(ip: ip, port: rpPort, timeoutMs: deviceProbeTimeout)
        if rpReachable {
            debugLog("[transport] RPPairing available: \(ip):\(rpPort)")
        } else {
            debugLog("[transport] RPPairing unavailable: endpoint \(rpPort) unreachable")
        }

        debugLog("[transport] probing Lockdown")
        let ldReachable = NetworkUtils.testTCP(ip: ip, port: ldPort, timeoutMs: deviceProbeTimeout)
        if ldReachable {
            debugLog("[transport] Lockdown available: \(ldPort)")
        } else {
            debugLog("[transport] Lockdown unavailable: endpoint \(ldPort) unreachable")
        }

        let caps = gateway.pairingCapabilities
        let type = gateway.pairingFileType
        let supportsRP = type == .unknown ? true : caps.supportsRemotePairing
        let supportsLD = type == .unknown ? true : caps.supportsLockdown
        let selected = DeviceTransportSelector.select(
            remotePairingReachable: rpReachable,
            lockdownReachable: ldReachable,
            pairingSupportsRemotePairing: supportsRP,
            pairingSupportsLockdown: supportsLD
        )
        debugLog("[transport] selected=\(selected)")
        let result = DeviceTransportProbeResult(
            remotePairingReachable: rpReachable,
            lockdownReachable: ldReachable,
            remotePairingPort: rpPort,
            lockdownPort: ldPort,
        ...
        gateway.recordTransportProbe(result)
        return result
    }

    nonisolated private func tcpProbe(_ ip: String?) -> Bool {
        probeDevice(ip).deviceReachable
    }
    ...
    func refresh(quietScan: Bool = false) async -> Bool {
        ...
        switch connectionMode {
            ...
            case .localVPN:
                ...
                let overrideProbe = probeDevice(overridePeerIp)
                isOverridePeerIpReachable = overrideProbe.deviceReachable
                lastProbe = overrideProbe
                if overridePeerIp == nil {
                    lastProbe = probeDevice(derivedPeerIp)
                }
            ...
            case .remoteServer:
                ...
                let probe = probeDevice(serverIp)
                let reachable = probe.deviceReachable
                lastProbe = probe

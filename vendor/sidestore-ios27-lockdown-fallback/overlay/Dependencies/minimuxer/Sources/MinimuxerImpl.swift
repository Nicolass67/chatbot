...
final internal class MinimuxerImpl: MinimuxerAPI {
    ...
    private func applyTransportFromProbe() async {
        guard getPairingFileType() != .unknown else { return }
        guard let probe = await connectionManager.lastProbe else { return }
        do {
            try gateway.applySelectedTransport(probe.selected)
        } catch {
            debugLog("[transport] applySelectedTransport failed: \(error)")
        }
        if gateway.pairingFileType == .lockdown {
            do {
                try await restartMuxerServer()
            } catch {
                debugLog("[transport] restartMuxerServer failed: \(error)")
            }
        }
    }
            ...
            case .localVPN:
                ...
                if self.gateway.pairingFileType != .rppairing && !net.isIKEv2IPSecAvailable {
                    if #available(iOS 26.4, *) {
                        if !self.gateway.lastLockdownReachable {
                            debugLog("[minimuxer] minimuxer not ready: no ipsec interface (required for lockdown on iOS 26.4+ when 62078 is unreachable)")
                        ...
                        debugLog("[minimuxer] IKEv2 not present; Lockdown TCP 62078 is reachable — continuing")
                    }
                }
    ...
    func debugApp(appId: String) async throws {
        try await matchingPriority{
            ...
            } catch let err as DeviceGatewayError where err.code == .requiresRemotePairing {
                debugLog("[transport] operation requires RPPairing")
                debugLog("[transport] RPPairing unavailable")
                throw MinimuxerError.requiresRemotePairing(err.reason)
            }
        }
    }

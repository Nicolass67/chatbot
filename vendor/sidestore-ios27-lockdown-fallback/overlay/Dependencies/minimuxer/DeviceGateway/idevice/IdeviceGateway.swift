...
public final class IdeviceGateway: BaseDeviceGateway, DeviceGatewayAPI {
    ...
    private var pairingHandleKind: PairingProtocol = .unknown
    ...
    private func cleanup() {
        ...
        pairingCapabilities = .none
        selectedTransport = .unavailable
        if let pairingFile = self.pairingFile {
            verboseLog("[IdeviceGateway] cleanup() freeing pairingFile kind=.\(pairingHandleKind)")
            if pairingHandleKind == .rppairing {
                rp_pairing_file_free(pairingFile)
            } else if pairingHandleKind == .lockdown {
                idevice_pairing_file_free(pairingFile)
            }
            self.pairingFile = nil
        }
        pairingHandleKind = .unknown
...
//        idevice_init_logger(IdeviceLogLevel(rawValue: 4), IdeviceLogLevel(rawValue: 0), nil)
    ...
    private func syncStart(pairingFileContent: String) throws {
        ...
        do {
            ...
            pairingCapabilities = PairingFileParser.capabilities(from: parsedPairingFile.plist)
            setPairingFileType(parsedPairingFile.mode)
            debugLog("[IdeviceGateway] start() parsed pairing mode=.\(parsedPairingFile.mode) supportsRP=\(pairingCapabilities.supportsRemotePairing) supportsLockdown=\(pairingCapabilities.supportsLockdown)")
        } catch {
    ...
    private func performWithEitherService<T>(
        ...
        if pairingFileType == .rppairing {
            do {
                ...
                let fallback = DeviceTransportSelector.resolveAfterRemotePairingFailure(
                    operation: .lockdownCompatible,
                    lockdownReachable: lastLockdownReachable || pairingCapabilities.supportsLockdown,
                    pairingSupportsLockdown: pairingCapabilities.supportsLockdown
                )
    ...
    private func syncFetchUDID() throws -> String? {
        ...
        debugLog("[device] fetchUDID transport=\(pairingFileType)")
        ...
        if pairingFileType == .rppairing {
            do {
                ...
                if case .use(.lockdown) = fallback {
                    debugLog("[transport] RPPairing unavailable: tunnel_create_rppairing failed")
                    debugLog("[transport] probing Lockdown")
                    try applySelectedTransport(.lockdown)
                    if let udid = try fetchUDIDViaLockdownTCP() {
                        debugLog("[device] UDID resolved")
                        return udid
                    }
                }
                return nil
            ...
            do {
                ...
                debugLog("[IdeviceGateway] fetchUDID lockdown TCP failed: \(error)")
            }
            return try fetchUDIDViaUsbmuxd()
        }
    }

    private func fetchUDIDViaLockdownTCP() throws -> String? {
        debugLog("[device] fetchUDID transport=lockdown")
        return try performWithTcpService(
            ...
            var plistVal: plist_t? = nil
            verboseLog("[IdeviceGateway] fetchUDID lockdown TCP calling lockdownd_get_value for UniqueDeviceID")
            let valErr = lockdownd_get_value(client, "UniqueDeviceID", nil, &plistVal)
            if let valErr = valErr {
                debugLog("[IdeviceGateway] fetchUDID lockdown TCP lockdownd_get_value failed")
                safeFreeError(valErr)
                return nil
            }
            defer { if let plistVal { safeFreePlist(plistVal) } }
            return plistVal.flatMap { getRustPlistString($0) }
        }
    }

    private func fetchUDIDViaUsbmuxd() throws -> String? {
        var conn: OpaquePointer? = nil
    ...
    private func syncDebugApp(appId: String) throws {
        ...
        if major < 17 {
            ...
            throw IdeviceGatewayError(
                ...
                reason: "JIT/debug on iOS \(major)+ requires RPPairing/RSD; Lockdown 62078 cannot start debug_proxy_connect_rsd"
            )
        } else {
            try performWithEitherService(
    ...
    private func syncDebugProcess(pid: UInt32) throws {
        ...
        if pairingFileType != .rppairing {
            ...
            throw IdeviceGatewayError(
                .requiresRemotePairing,
                reason: "attachDebugger requires RPPairing/RSD debug proxy"
            )
        }
        try performWithEitherService(
            connectRP: debug_proxy_connect_rsd,

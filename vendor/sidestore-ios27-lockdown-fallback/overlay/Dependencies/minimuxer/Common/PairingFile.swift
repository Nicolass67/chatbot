...
public struct PairingFileCapabilities: Equatable, Sendable {
    public let supportsRemotePairing: Bool
    public let supportsLockdown: Bool
    public let lockdownUDID: String?

    public static let none = PairingFileCapabilities(
        supportsRemotePairing: false,
        supportsLockdown: false,
        lockdownUDID: nil
    )
}

public enum PairingFileParser {
    /// Independent of which protocol `parse()` prefers. A hybrid file (lockdown + RP keys)
    /// supports both even though `parse()` currently returns `.rppairing` first.
    public static func capabilities(from plist: [String: any Sendable]?) -> PairingFileCapabilities {
        guard let plist else { return .none }
        let supportsRemotePairing = RPPairingFile.missingKeys(in: plist).isEmpty
        let supportsLockdown = LockdownPairingFile.missingKeys(in: plist).isEmpty
        let udid = plist["UDID"] as? String
        return PairingFileCapabilities(
            supportsRemotePairing: supportsRemotePairing,
            supportsLockdown: supportsLockdown,
            lockdownUDID: (udid?.isEmpty == false) ? udid : nil
        )
    }

    public static func capabilities(from content: String) -> PairingFileCapabilities {
        guard let data = content.data(using: .utf8),
              let rawPlist = try? PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [String: Any] else {
            return .none
        }
        return capabilities(from: toSendableDictionary(rawPlist))
    }
